...
instance I.Nonce PKNonce where
  ...
  size = Tagged pkNonceLength
  toBS (PKNonce b)   = b
  fromBS x
    | S.length x == pkNonceLength = Just (PKNonce x)
    | otherwise                   = Nothing
...
encrypt (PKNonce n) msg (Key pk) (Key sk) = unsafePerformIO $ do
  ...
  return $ SU.unsafeDrop pk_msg_BOXZEROBYTES r
{-# INLINEABLE encrypt #-}
...
decrypt (PKNonce n) cipher (Key pk) (Key sk) = unsafePerformIO $ do
  ...
  return $ if r /= 0 then Nothing
             ...
             in Just $ SU.unsafeDrop pk_msg_ZEROBYTES bs
{-# INLINEABLE decrypt #-}
...
-- bytes.
publicKeyLength :: Int
publicKeyLength  = pkPublicKeyLength

-- | Length of a 'Crypto.NaCl.Key.Secret' signing 'Crypto.NaCl.Key.Key' in
-- bytes.
secretKeyLength :: Int
secretKeyLength  = pkSecretKeyLength
...
prepEnc bs = do
  c <- SI.mallocByteString (S.length bs + pk_msg_ZEROBYTES)
  return $! (c, S.replicate pk_msg_ZEROBYTES 0x0 `S.append` bs)

prepDec :: ByteString -> IO (ForeignPtr Word8, ByteString)
prepDec bs = do
  c <- SI.mallocByteString (S.length bs + pk_msg_BOXZEROBYTES)
  return $! (c, S.replicate pk_msg_BOXZEROBYTES 0x0 `S.append` bs)
